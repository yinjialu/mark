...
长任务中的较早消息可能暂时不在页面里，定位时会先通过原生导航加载对应轮次，再查找具体标记。v0.1.6 起，即使消息容器已经出现，也会继续等待具体文字选区、图片、图表或表格完成挂载；内容未变化时定位到准确位置，精确匹配失效时才退到对应消息或轮次。
...
更新到 v0.1.6。此前消息容器先于具体图表或选区挂载时，导航会误以为定位已经完成；新版会先把消息带入可见区域，并继续等待准确位置。原文内容已经变化时仍会退到消息或轮次，并在页面中显示提示。
