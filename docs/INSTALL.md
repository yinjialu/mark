...
- 正式客户端升级后，运行 `~/Applications/mark.app`。启动器会读取新的正式版：精确适配可用时先生成新副本再切换；没有适配器或生成失败时直接打开上一可用副本，收藏页显示等待适配。无需降级、替换或阻止正式版更新。
- ChatGPT mark 副本关闭官方自更新流程，避免本地签名副本下载更新后安装失败。在收藏页点 **升级 ChatGPT**，mark 会验证原始客户端仍有 OpenAI 签名，再正常退出当前副本并打开正式版。完成官方更新后重新打开 `~/Applications/mark.app`，启动器会同步适配；切换正式版失败则恢复当前副本。
