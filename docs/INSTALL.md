...
- 首次通过 `~/Applications/ChatGPT mark.app` 启动，将运行中的 **ChatGPT mark** 固定到 Dock；日常从 Dock 打开。完整步骤见[使用指南](USAGE.md#日常只用一个入口)。
...
- 成功升级或回退后，已有的 Dock mark 入口自动更新；安装器保留当前副本、待切换副本和一个回退副本，并清理无引用的历史构建与安装包。
- 正式客户端升级后，运行 `~/Applications/ChatGPT mark.app`。启动器会读取新的正式版：精确适配可用时先生成新副本再切换；没有适配器或生成失败时直接打开上一可用副本，收藏页显示等待适配。无需降级、替换或阻止正式版更新。
- ChatGPT mark 副本关闭官方自更新流程，避免本地签名副本下载更新后安装失败。在收藏页点 **升级 ChatGPT**，mark 会验证原始客户端仍有 OpenAI 签名，再正常退出当前副本并打开正式版。完成官方更新后重新打开 `~/Applications/ChatGPT mark.app`，启动器会同步适配；切换正式版失败则恢复当前副本。
- 更新状态同时比较 mark 发布版本和副本所基于的正式客户端。即使 mark 版本号相同，只要正式客户端已变化且该版本已有精确适配，也会提供 **适配新版并重启**，不会误报“已是最新”。
