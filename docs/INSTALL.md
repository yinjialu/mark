...
```text
请帮我安装 mark：https://github.com/yinjialu/mark ，使用 v0.1.0-preview.4 标签。
...
从 [预览版发布页](https://github.com/yinjialu/mark/releases/tag/v0.1.0-preview.4) 下载 ZIP 和 SHA256，校验后解压，双击 `Install.command`，输入 `install`。

也可以在终端执行（目标目录须不存在）：

```sh
git clone --branch v0.1.0-preview.4 --depth 1 https://github.com/yinjialu/mark.git "$HOME/mark"
cd "$HOME/mark"
