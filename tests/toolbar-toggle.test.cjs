...
const code=fs.readFileSync(path.join(__dirname,'../integration/src/toolbar-button.js'),'utf8');
...
function setup(){
 ...
 const context={xlr:R,g3:{jsx:(type,p)=>p},zH:'Button',window:win,Event,crypto:webcrypto,TextEncoder,Uint8Array};vm.createContext(context);vm.runInContext(code,context);
