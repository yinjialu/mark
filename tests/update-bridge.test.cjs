...
test('update IPC only accepts fixed operations and opaque tickets',()=>{
 assert(validRequest({op:'check',force:false}));assert(validRequest({op:'status'}));assert(validRequest({op:'official'}));assert(validRequest({op:'install',ticket:'11111111-2222-3333-4444-555555555555'}));
