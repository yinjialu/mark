...
with tempfile.TemporaryDirectory(prefix='mark-launcher-check-') as tmp:
 ...
 info=plistlib.loads((launcher/'Contents/Info.plist').read_bytes());info['CFBundleIdentifier']='local.mark.launcher.check';(launcher/'Contents/Info.plist').write_bytes(plistlib.dumps(info))
