...
6. 更新发布版本，重新生成 SHA256SUMS；扩大支持范围前，在独立机器完成安装和实际客户端点击验收。每次发布必须列出尚未完成的验收；v0.1.0 按维护者决定结束预览编号，支持范围仍严格限定于当前清单，第二台 Mac 首装验收尚未完成。
...
`tests/browser/library-page.js` 是不含宿主代码的浏览器回归脚本。它要求开发环境先在独立页面载入本机提取的原生组件测试绑定（`__native`、`__rail`、`__sidebarItem`、`createMarkSidebar`、`createMarksUI`、`summaryOf`、`sha`），覆盖页面边界、搜索、原生标签键盘操作、片段/上下文切换、编辑保存、回收站和来源定位。该脚本不属于 Node 单元测试入口；不要在正在使用的客户端中注入。
